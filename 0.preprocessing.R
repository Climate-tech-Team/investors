...
investors.i3<-read_excel(paste(getwd(),"/input/i3raw/20220119_i3rawdata/investors-export-18921.xls",sep=""),na = "") %>%
 mutate_all(tolower) %>%
...
investments.i3<-read_excel(paste(getwd(),"/input/i3raw/20220119_i3rawdata/investors-export-18920.xls",sep=""),
                           sheet=2, na = "") %>%
  mutate_all(tolower) %>%
  dplyr::rename_all(funs(make.names(.)))%>%
  rename_all(tolower) %>%
  ...
  select(investor.firm,company,i3.url,year.founded,industry.group,sector,tags,short.description,state, country,region,investment.type,investment.year,investment.date,investment.amount,
         total.paid.in.capital,year.founded,ticker.symbol)
...
investments.combined<-left_join(investments.i3,investors.i3, by="investor.firm") %>%
  mutate(year.founded = ifelse(year.founded>2022 | is.na(year.founded)==TRUE, 2100, year.founded))


# data verified by collaborators
startup.year1<-read_excel(paste(getwd(),"/input/foundingyr/year_kurowski.xls",sep=""),na = "") %>%
  mutate_all(tolower)%>%
...
startup.year2<-read_excel(paste(getwd(),"/input/foundingyr/year_authors_2022.xlsx",sep=""),na = "") %>%
  mutate_all(tolower)%>%
...
investments.combined<-left_join(investments.combined,startup.year) %>%
  mutate(year.founded = ifelse(is.na(year.founded.update)==TRUE,year.founded,year.founded.update)) %>%
...
#companies excluded because the sector is "other cleantech" and not explicitly linked to climate------

investments.other<-investments.combined %>%
  select(company, industry.group,sector,tags,tags.filter,short.description,year.founded,region,ticker.symbol) %>%
  filter(sector=="other cleantech") %>%
  filter(!str_detect(tags.filter, included.tags)) %>%
  unique() %>%
...
investments.unspecified<-investments.combined %>%
  ...
  filter(is.na(sector)==TRUE | sector == ("other cleantech") | industry.group == ("enabling technologies")) %>%
  filter(!str_detect(tags.filter, included.tags)) %>%
  unique() %>%
...
#companies excluded because they were founded before 2005 or no year------
...
investments.unis<-investments.combined %>%
  select(company, industry.group,sector,tags,tags.filter,short.description,year.founded,region,ticker.symbol) %>%
  filter(str_detect(company, c("university|research institute|institute of technology|polytechnique|polytechnic|national laboratory|research laboratory|government-owned|public agency|public utility|research center|research centre|not-for-profit|not for profit|non-profit")) | 
           str_detect(short.description, c("research institute|institute of technology|polytechnique|polytechnic|national laboratory|research laboratory|government-owned|public agency|public utility|research center|research centre|not-for-profit|not for profit|non-profit"))) %>%
  unique() %>%
...
manual.edits <- "hippo insurance|pecan street|roomorama|scale ai|mapr technologies|top flight technologies|intelligent layer|8 rivers capital|rappaport energy consulting|exxon mobil"
...
write.csv(investments.exclude,paste(getwd(),"/exclude/companies-exclude-20220311.csv",sep=""),fileEncoding = "UTF-8")
