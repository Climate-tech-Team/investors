...
investments.exclude<-read.csv(paste(getwd(),"/exclude/companies-exclude-20220311.csv",sep=""),fileEncoding = "UTF-8") %>%
  select(-X) %>%
  select(-reason) %>%
  unique() %>%
...
investments.combined<-left_join(investments.i3,investments.exclude) %>%
...
investments.combined<-left_join(investments.combined,startup.year) %>%
  mutate(year.founded.new = ifelse(is.na(year.founded.update)==TRUE,year.founded,year.founded.update)) %>%
...
investments.combined <- investments.combined %>%
  ...
  mutate(pre2005investments.amt = sum(investment.amount.revised[investment.year<2005],na.rm=TRUE))%>%
  mutate(post2005investments.amt = sum(investment.amount.revised[investment.year>=2005],na.rm=TRUE))%>%
  ungroup() %>%
...
firms.list<- investments.combined %>%
  select(company,i3.url,year.founded,state,country,region,sector,tags,short.description,pre2005investments.n,post2005investments.n,
         pre2005investments.amt,post2005investments.amt) %>%
  unique()
...
write.csv(investors.list,paste(getwd(),"/input/i3manualcheck/investors-20220311.csv",sep=""),fileEncoding = "UTF-8")

#export list of startups
write.csv(firms.list,paste(getwd(),"/input/i3manualcheck/firms.list-20220311.csv",sep=""),fileEncoding = "UTF-8")
...
investors.cleaned<-read_excel(paste(getwd(),"/input/i3manualcheck/investors-cleaned-20220504.xlsx",sep=""),na = "") %>%
  select(1:5)
...
investors.tmp<-investors.cleaned %>%
  ...
  unique() %>%
  group_by(investor.firm.renamed, investor.type.edited,investor.country.edited) %>%
  mutate(investor.id = cur_group_id()) %>% #unique id for each investor
  ungroup() %>%
  unique()
  
#combining with cleaned i3 dataset
investors<-left_join(investors.tmp,investments.combined, by=c("investor.firm")) %>%
...
#write.csv(investors.corp,paste(getwd(),"/input/i3manualcheck/investors.cpc-20220311.csv",sep=""),fileEncoding = "UTF-8")
...
stoplist<-c(" ltd| plc| inc| llc| co inc| s e| s a b|s a| s p a| holdings| group| corporation| gmbh| sa| management| holding| se| nv| n v| ag| investors| limited| il| plc| spa| incorporated| corporation| corp| a s| company")
targets.sbti<-read.csv(paste(getwd(),"/input/sbti/companies-taking-action.csv",sep="")) %>%
  dplyr::rename_all(funs(make.names(.)))%>%
  rename_all(tolower) %>%
  mutate_all(tolower) %>%
...
investors.sbti <- investors %>%
  mutate(investor.firm = investor.firm.renamed) %>%
  rbind(investors) %>%
  select(investor.firm,investor.country,investor.firm.renamed,investor.country.edited,investor.type.edited,investor.id) %>%
  unique() 
  #filter(investor.type.edited == "corporation or corporate venture")
...
investors.sbti.matched <-investors.sbti %>%
  ...
  select(-sbti.date.explanation, -sbti.target, -sbti.target.classification,-sbti.sme.) %>%
  group_by(investor.id) %>%
...
investors<-left_join(investors,investors.sbti.matched,by="investor.id") %>%
  unique()

remove(stoplist,targets.sbti,investors.sbti,investors.sbti.matched,investors.tbc)
...
investor.category<-read.csv(paste(getwd(),"/input/investortype/investors-type.csv",sep="")) %>%
  dplyr::rename_all(funs(make.names(.)))%>%
  rename_all(tolower) %>%
  mutate_all(tolower) 
...
write.csv(investors,paste(getwd(),"/output/investors_20220504.csv",sep=""),fileEncoding = "UTF-8")

#3.27 edits: corrected toyota country which was france in some cases
#3.27 edits: microsoft is now under digital
# shell accelerator corrected
# chain reaction innovation corrected
# koch strategic corrected
